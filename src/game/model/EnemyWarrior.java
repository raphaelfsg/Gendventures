...
public class EnemyWarrior extends Character implements Ifight, IdamageSystem, Iequips {
    ...
    public EnemyWarrior(int strength, int speed, int intelligence, int life, int weaponDamage, int armorDefense) {
    ...
    public void defenseMsg() {
        System.out.println("Olha o dano manooooo " + defense());
    }
    @Override
    public int defense() {
        double sorte = Math.random();
        if (sorte < 0.98) {
            return life -= (getTotalDamage() / 30);
        } else {
            life -= (getTotalDamage() / 50);
            return defense();
        }
